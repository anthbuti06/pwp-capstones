...
def get_average_sentence_length(text):
    text_fixed1 = text.replace("?", ".")
    text_fixed2 = text_fixed1.replace("!", ".")
    sentences_in_text = text_fixed2.split(".")
    sentence_lengths = []
    ...
    for length in sentence_lengths:
        word_count += length
    average = float(word_count/ len(sentence_lengths))
    return average
...
class TextSample:
    ...
    def __repr__(self):
        return "Author:{author}, Average sentence length:{average}".format(author=self.author, average=self.average_sentence_length)
...
def frequency_comparison(table1, table2):
    ...
    for entry in table1.keys():
        if entry in table2.keys():
            if table1[entry] < table2[entry]:
                mutual_appearances += table1[entry]
                appearances += table2[entry]
            else:
                mutual_appearances += table2[entry]
                appearances += table1[entry]
        else:
            appearances += table1[entry]
    for entry in table2.keys():
        if not entry in table1.keys():
            appearances += table2[entry]
